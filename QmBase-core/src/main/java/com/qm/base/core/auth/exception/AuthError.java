...
public enum AuthError implements ICode {
    ...
    AUTH_ACCOUNT_OR_PASSWORD_ERROR("AUTH_ACCOUNT_OR_PASSWORD_ERROR", "账号或密码错误", HttpStatus.BAD_REQUEST),
    ...
    AUTH_CREDENTIAL_INVALID("AUTH_CREDENTIAL_INVALID", "密码格式错误，最少8位，必须包含大小写字母、数字，允许特殊字符", HttpStatus.BAD_REQUEST),
    ...
    // 系统相关
    AUTH_ERROR("AUTH_ERROR", "认证异常", HttpStatus.INTERNAL_SERVER_ERROR),
    AUTH_REQUEST_ERROR("AUTH_REQUEST_ERROR", "请求参数错误", HttpStatus.BAD_REQUEST),
    // Token 相关
    AUTH_TOKEN_EMPTY("AUTH_TOKEN_EMPTY", "令牌为空", HttpStatus.UNAUTHORIZED),
    AUTH_TOKEN_INVALID("AUTH_TOKEN_INVALID", "令牌无效", HttpStatus.UNAUTHORIZED),
    AUTH_TOKEN_EXPIRED("AUTH_TOKEN_EXPIRED", "令牌过期", HttpStatus.UNAUTHORIZED),
    AUTH_TOKEN_MISSING("AUTH_TOKEN_MISSING", "缺少认证令牌", HttpStatus.UNAUTHORIZED),
